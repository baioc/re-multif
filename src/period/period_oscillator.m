...
% ============================ INPUT PARAMETERS ================================
    ...
    % input vector
    Is = 1e-9 * 5; % constant input concentration
...
% =============================== SIMULATION ===================================
    ...
    end_time = -1;
    period_time = -1;
    period_vector = zeros(1, 75);
    concentration_vector = zeros(1, 75);
    % actual simulation
    for s = 1 : 75
        Is = (0.5 + 0.1 * (s - 2));
        concentration_vector(s) = Is;
        Is = Is * 1e-9;
        for t = 1 : length(simulation)
            ...
            if R3 > 15e-9
                if is_period == 0
                    ...
                    if begin_time == -1
                        ...
                        end_time = t;
                        period_time = (end_time - begin_time) * 60;
                        begin_time = -1;
                    endif
                endif
            endif
            if R3 < 5e-9 && is_period == 1
                is_period = 0;
            endif

            % common subexpression optimization (used below)
            activationP1 = Ha(Is, Na_P1, Ka_P1);
            repressionP1 = Hr(R2, Nr_R2P1, Kr_R2P1);
            activationP2 = Ha(Is, Na_P2, Ka_P2);
            ...
            % compute variation
            dR1dt = a1_prime + a2_prime - a3_prime;
            dR2dt = b1_prime + b2_prime - b3_prime;
            dR3dt = c1_prime + c2_prime - c3_prime;
            dR4dt = d1_prime + d2_prime - d3_prime;
...
% ================================ RESULTS =====================================
    ...
    % model subplot
    plot(concentration_vector,period_vector,'-b;Period;');
    %plot(x,yR1,'-m;R1;', x,yR2,'-k;R2;', x,yR3,'-r;R3;', x,yR4,'-g;R4;');
    xlabel("Concentration (nM)");
    ylabel("Period (seconds)");
    title("Oscillatory Stochastics");

    hold off;
    print('stochastic-oscillator.pdf'); % put in the folder the script is run from
    xd = input("\nPress enter to exit ");
