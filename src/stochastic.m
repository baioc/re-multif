...
function dR = stochastic(t, R, I, N, Ha, Hr, k_a, k_r, k_tl, beta, P_tc, delta_x, delta_m)
  ...
  dR(1) = a1 + a2 - a3 + (sqrt(a1)*N(t,a1_idx) + sqrt(a2)*N(t,a2_idx) - sqrt(a3)*N(t,a3_idx)) / sqrt(V);

  dR(2) = b1 + b2 - b3 + (sqrt(b1)*N(t,b1_idx) + sqrt(b2)*N(t,b2_idx) - sqrt(b3)*N(t,b3_idx)) / sqrt(V);

  dR(3) = c1 + c2 - c3 + (sqrt(c1)*N(t,c1_idx) + sqrt(c2)*N(t,c2_idx) - sqrt(c3)*N(t,c3_idx)) / sqrt(V);

  dR(4) = d1 + d2 - d3 + (sqrt(d1)*N(t,d1_idx) + sqrt(d2)*N(t,d2_idx) - sqrt(d3)*N(t,d3_idx)) / sqrt(V);
